...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Стр Из Начисления Цикл
        Если Стр.ДатаНачала > Стр.ДатаОкончания ИЛИ Месяц(Стр.ДатаНачала) <> Месяц(Стр.ДатаОкончания) Тогда
			...
			Сообщение.Текст = СтрШаблон("Строка №%1 не верно задон период!!!", Стр.НомерСтроки);
			Сообщение.Сообщить(); 
            Отказ = Истина; 
		КонецЕсли;
		...
		Если Стр.ВидРасчета = ПланыВидовРасчета.ВКМ_ДополнительныеНачисления.Премия Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон("Строка №%1 не верный вид расчета. Премия начисляется отдельным документом!!!", Стр.НомерСтроки);
			Сообщение.Сообщить();
            Отказ = Истина;
		КонецЕсли; 
		...
		Если Месяц(Стр.ДатаНачала) <> Месяц(Дата) Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтрШаблон("Строка №%1 месяц документа не равен месяцу начала начислению!!!", Стр.НомерСтроки);
			Сообщение.Сообщить();
			Отказ = Истина;
		КонецЕсли;
	КонецЦикла;  
...
Процедура ВКМ_РассчитатьОклад();
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл  
		...
		Если ВыборкаДетальныеЗаписи.Норма = 0 Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Не заполнен график работ!!! Не возможно проведение!!!";
			Сообщение.Сообщить();
			Отказ = Истина; 
			Прервать;
		КонецЕсли;
