...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Договор.ВКМ_НачалаДействия > Дата ИЛИ  Договор.ВКМ_ОкончанияДействия < Дата Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтрШаблон("Не действующий договор: %1. Не удалось провести документ.", Договор);
		Сообщение.Сообщить();
		Отказ = Истина; 
	КонецЕсли; 
	// ВКМ_}
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения) 
	...
	Если Ссылка.ДатаПроведенияРабот <> ДатаПроведенияРабот Или 
		Ссылка.ВремяНачалаРабот <> ВремяНачалаРабот Или 
		Ссылка.Специалист <> Специалист Тогда
...
Процедура ПриЗаписи(Отказ)
	...
	КонецЕсли;
	//ВКМ_Телеграм.ВКМ_ТелеграмУведомление();   //для проверки УДАЛИТЬ
    //ВКМ_} 
