	...
	Для каждого Стр Из ОтпускаСотрудников Цикл   
		...
		Если ЗначениеЗаполнено(Стр.ДатаНачала) И ЗначениеЗаполнено(Стр.ДатаОкончания) Тогда 
			Если Год(Стр.ДатаНачала) <> Год(ГодПериод) Или Год(Стр.ДатаОкончания) <> Год(ГодПериод) Тогда
				...
				Сообщение.Текст = СтрШаблон("Строка №%1 год документа отличается от года в строке.", Стр.НомерСтроки);
				Сообщение.Сообщить();
				Отказ = Истина;
			КонецЕсли;
			
			Если Стр.ДатаНачала > Стр.ДатаОкончания Тогда  
				Сообщить(СтрШаблон("Строка №%1 не верно заполнен период", Стр.НомерСтроки)); 
				Отказ = Истина; 
			КонецЕсли; 
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Дубль Тогда
		Сообщить(СтрШаблон("План графиков за %1 год уже создан",  XMLСтрока(Год(ГодПериод))));
		Отказ = Истина;
	КонецЕсли;
